...
// 배열? 객체?에 넣고싶은데 어케 하는지 모르겠으니 일단 빼서 쓰자 - 2023.10.18(수)

// 필요한 값 추출!
// console.log(originData.results[1].title);

// HTML에 영화 카드 추가
let selectData = () => {
    for (let i = 0; i < originData.results.length; i++) {
        // 필요한 값 추출 후 변수에 참조!!
        let title = originData.results[i].original_title;
        let overview = originData.results[i].overview;
        let vote_average = originData.results[i].vote_average;
